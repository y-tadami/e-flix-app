...
## Step 1: 動画ファイルを Google Drive にアップロード
...
## Step 3: DriveリンクをiFrame埋め込み用に変換

コピーしたURLの末尾を `/view?usp=sharing` から `/preview` に書き換えます。
...
変換後: https://drive.google.com/file/d/1abc123XYZ/preview   ← これをスプシに記入
```

---

## Step 4: スプレッドシートに情報を記入
...
| `driveLink` | ✅ | Step 3 で変換した `/preview` URL | `https://drive.google.com/file/d/.../preview` |
...
> ⚠️ カテゴリが一致しない場合、「すべて」には表示されますが各カテゴリタブには表示されません。
...
| 症状 | 原因 | 対処 |
|------|------|------|
| 動画が再生されない | Driveリンクがpreviewになっていない | Step 3 のURL変換を確認 |
| 動画が再生されない | Google Driveの共有設定が非公開 | Step 2 の共有設定を確認 |
